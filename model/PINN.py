...
Inputs:
    ...
    *** Optimizers ***
    ...
    - iter_lbfgs         : Number of iterations for L-BFGS steps
...
def trainPINN(all_params, t, lb, ub, data, lamda = (1.0, 1.0, 1.0),\
    ...
    if None == opt_state:
        opt_state = adam_init(all_params)
    ...
    for i in range(iter_adam):
        ...
        losses, g = value_and_grad(loss_fn, argnums = 0, has_aux = True)(\
                                        updated_params, t, v_true, x_true,\
                                        lb, ub, lamda)
    ...
    for i in range(iter_lbfgs):
        # Optimizer Step - Weights and States Update
        updated_params, opt_state = lbfgs_solver.run(updated_params, t, v_true, x_true,\
                                                    lb, ub, lamda)
...
def plot_trajectory_PINN(x_net, params, t, x_data,\
    ...
    print('Number Test Prediction Points: ', num_test)
    ...
    np.save('./PredictionLosses/PINNloss', np.array(MSE_losses))
    
    if savefig:
        plt.savefig(join('./PredictionLosses', text + '.pdf'))
    #plt.show()
    plt.close()
    return MSE_losses, x_pred
